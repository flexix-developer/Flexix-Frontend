...
const LayerExplorer = () => {
  ...
  const findAllMainDivTags = () => root.querySelector('#main')?.childNodes?.filter(node => node.nodeType === 1) || null;

  const RenderContentItem = ({ tag, depth }) => {
    const [isExpanded, setIsExpanded] = useState(true);
    const toggleExpansion = () => setIsExpanded(!isExpanded);
    const hasNonEmptyChildNodes = tag.childNodes.some(child => child.nodeType === 1 && child.rawTagName !== "script");

    const iconMapping = {
      DIV: {
        check: (tag) => tag.id && (tag.id.toLowerCase().startsWith('col') || tag.id.toLowerCase().includes('row')),
        icon: (
          <>
      ...
      LABEL: { check: () => true, icon: <MdOutlineLabel size={20} className="text-white-500" /> },
      BUTTON: { check: () => true, icon: <RxButton size={20} className="text-white-500" /> },
      INPUT: { check: () => true, icon: <MdInput size={20} className="text-white-500" /> },
      TEXTAREA: { check: () => true, icon: <BsTextareaResize size={20} className="text-white-500" /> },
      SELECT: { check: () => true, icon: <RxDropdownMenu size={20} className="text-white-500" /> },
    };
    

    const tagType = Object.keys(iconMapping).find(key => tag.tagName === key);
    const tagIcon = iconMapping[tagType]?.check(tag) && iconMapping[tagType]?.icon;
    ...
    return (
      <div className="border-l border-gray-300 p-2">
        <div className="flex items-center">
          {hasNonEmptyChildNodes && (
            <span onClick={toggleExpansion} className="cursor-pointer">
              {isExpanded ? <SlArrowDown className="transform rotate-0" /> : <SlArrowRight className="transform rotate-0" />}
            </span>
          )}
        ...
        <div className={`ml-2 transition-all duration-300 overflow-hidden ${isExpanded ? "block" : "h-0"}`}>
          {hasNonEmptyChildNodes &&
            tag.childNodes
              .filter(node => node.nodeType === 1 && node.rawTagName !== "script" && node.rawTagName !== "style")
              .map((child, index) => <RenderContentItem key={index} tag={child} depth={depth + 1} />)
          }
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className="flex flex-col text-white items-start mt-4 ml-2">
        {mainDivTags && mainDivTags.map((tag, index) => <RenderContentItem key={index} tag={tag} depth={0} />)}
      </div>
    </div>
  );
};
