      ...
      return (
        <div>
          ...
          {lines.length > 2 && (
            <div>
              {showFullDescription ? (
                <span className="text-blue-200 cursor-pointer" onClick={handleSeeLessClick}>
                  See less ...
                </span>
              ) : (
                <span className="text-blue-200 cursor-pointer" onClick={handleSeeMoreClick}>
                  See more ...
                </span>
              )}
            </div>
    ...
    <div className="h-full bg-gray-900">
      ...
      <div className="flex flex-row mx-8">
        <div className="w-3/12 flex flex-col mr-1">
          <button
            onClick={() => handleButtonClick('https://www.youtube.com/embed/JWJul9o1qwM?si=JJFmaFQLCU1_X91n')}
            className="bg-white p-4 mb-4 rounded text-blue-700 font-bold text-lg hover:bg-gray-100 cursor-pointer text-left inline-block w-auto"
          >
            01. วิธีการสร้างและการตั้งชื่อโปรเจกต์
          </button>
          <button
            onClick={() => handleButtonClick('https://www.youtube.com/embed/MoGDiI_K-Pk?si=ulxZ1Svbd3ukMZ81')}
            className="bg-white p-4 mb-4 rounded text-blue-700 font-bold text-lg hover:bg-gray-100 cursor-pointer text-left inline-block w-auto"
          >
            02. แนะนำ Toolbox และ Properties ต่างๆ
          </button>
          <button
            onClick={() => handleButtonClick('https://www.youtube.com/embed/27afGUnAqGo?si=7dwe7VCBnvH3VCke')}
            className="bg-white p-4 mb-4 rounded text-blue-700 font-bold text-lg hover:bg-gray-100 cursor-pointer text-left inline-block w-auto"
          >
            03. แนะนำการวาง widgets วางๆ ใน Toolbox
          </button>
          <button
            onClick={() => handleButtonClick('https://www.youtube.com/embed/HDroWRz4n54?si=RhJJ0U_qOmw2OkWQ')}
            className="bg-white p-4 mb-4 rounded text-blue-700 font-bold text-lg hover:bg-gray-100 cursor-pointer text-left inline-block w-auto"
          >
            04. แนะนำการใช้งาน API และการจัดเตรียม API
          </button>
          <button
            onClick={() => handleButtonClick('https://www.youtube.com/embed/Az893xQQLk8?si=UVDSVBnPjNcDPLX6')}
            className="bg-white p-4 mb-4 rounded text-blue-700 font-bold text-lg hover:bg-gray-100 cursor-pointer text-left inline-block w-auto"
          >
