...
const initialState = {
    value: '<html><head><title>Document</title></head><body><div class="main"></div></body></html>',
    currentRowNumber: 0,
    currentColNumber: 0,
    currentTextNumber: 0,
    currentLinkNumber: 0,
...
export const counterSlice = createSlice({
  ...
  reducers: {
    ...
    addImage: (state) => {
        ...
        const image = parse(`<img class="image-${state.currentImageNumber}" src="https://via.placeholder.com/150">`);
        root.querySelector(".main").appendChild(image);
        state.value = root.toString();
    },
    addEmbed: (state) => {
        state.currentEmbedNumber += 1;
        const embed = parse(`<iframe class="embed-${state.currentEmbedNumber}" src="https://www.youtube.com/embed/tgbNymZ7vqY"></iframe>`);
        root.querySelector(".main").appendChild(embed);
        state.value = root.toString();
    },
    addForm: (state) => {
        state.currentFormNumber += 1;
        const form = parse(`<form class="form-${state.currentFormNumber}"></form>`);
        root.querySelector(".main").appendChild(form);
        state.value = root.toString();
    },
    addLabel: (state) => {
        state.currentLabelNumber += 1;
        const label = parse(`<label class="label-${state.currentLabelNumber}">Label</label>`);
        root.querySelector(".main").appendChild(label);
        state.value = root.toString();
    },
    addButton: (state) => {
        state.currentButtonNumber += 1;
        const button = parse(`<button class="button-${state.currentButtonNumber}">Button</button>`);
        root.querySelector(".main").appendChild(button);
        state.value = root.toString();
    },
    addTextarea: (state) => {
        state.currentTextareaNumber += 1;
        const textarea = parse(`<textarea class="textarea-${state.currentTextareaNumber}"></textarea>`);
        root.querySelector(".main").appendChild(textarea);
        state.value = root.toString();
    },
    addInput: (state) => {
        state.currentInputNumber += 1;
        const input = parse(`<input type="text" class="input-${state.currentInputNumber}">`);
        root.querySelector(".main").appendChild(input);
        state.value = root.toString();
    },
    addSelect: (state) => {
        state.currentSelectNumber += 1;
        const select = parse(`<select class="select-${state.currentSelectNumber}"></select>`);
...
export const { addRow, addCol, addText, addLink, addImage, addEmbed, addForm, addLabel, addButton, addTextarea, addInput, addSelect } = counterSlice.actions
